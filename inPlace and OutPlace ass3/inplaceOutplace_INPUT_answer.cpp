...
void insertion_sort_InP(int arr[],int n){
...
void insertion_sort_OutP(int arr[],int si,int ei){
    //si=starting index
    //ei=ending index
    if(si==ei){return;}
    int j=si+1;
    ...
    insertion_sort_OutP(arr,si+1,ei);
}
...
int main(){
    ...
    cout<<endl;
    cout<<"ARRAY BEFORE SORTING : ";printfun(arr,n);
    //Calling insertion sort using in-place algorithm.
    insertion_sort_InP(arr,n);
    cout<<endl;
    ...
    insertion_sort_OutP(arr2,0,n-1);
